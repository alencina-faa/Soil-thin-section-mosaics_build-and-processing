...
def binary_tab(self):
    ...
    self.load_button = ttk.Button(
        self.binary_frame_controls, text="Load Images", command=lambda: load_image(self)
    )
    ...
    for i, name in enumerate(self.layer_names):
        ...
        visibility_cb = ttk.Checkbutton(
            layer_frame, variable=self.layer_visibility[i], command=lambda: update_display(self)
        )
        ...
        up_button = ttk.Button(
            layer_frame, text="↑", width=2, command=lambda idx=i: move_layer_up_by_index(self, idx)
        )
    ...
    self.save_button = ttk.Button(
        self.save_frame, text="Save Binary mosaic", command=lambda: save_image(self)
    )
