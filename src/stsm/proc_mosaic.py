...
def process_mosaic(self, image):
    ...
    cv2.drawContours(
        all_contours_image, all_contours, -1, (0, 0, 255), 2
    )  # Red color for all contours
...
def enhanced_process_mosaic_optimized(image):
    ...
    for idx, (parent_idx, is_edge) in enumerate(zip(parent_indices, is_edge_contour)):
        ...
        processed_contours.append(
            [idx, is_edge, parent, child_contours, final_area, final_perimeter]  # Edge flag
        )
...
def proc_cont_all(self):
    ...
    num_parent_contours_less_50 = sum(
        [1 for contour in self.processed_contours if contour[4] <= self.area_50]
    )

    # Calculate the number of child contours with area greater than 50 micron
    num_child_contours_less_50 = sum(
        [len(contour[3]) for contour in self.processed_contours if contour[4] <= self.area_50]
    )

    # Calculate the total area of contours with diameter less than 50 micron
    cont_total_area_less_50 = sum(
        [contour[4] for contour in self.processed_contours if contour[4] <= self.area_50]
    )

    # GREATHER THAN 50 micron
    # Calculate the number of parent contours with area greater than 50 micron
    num_parent_contours_great_50 = sum(
        [1 for contour in self.processed_contours if contour[4] > self.area_50]
    )

    # Calculate the number of child contours with area greater than 50 micron
    num_child_contours_great_50 = sum(
        [len(contour[3]) for contour in self.processed_contours if contour[4] > self.area_50]
    )

    # Calculate the total area of contours with diameter greather than 50 micron
    cont_total_area_great_50 = sum(
        [contour[4] for contour in self.processed_contours if contour[4] > self.area_50]
    )
    # ------------------------------------------------
        ...
        for size in self.sizes:
            ...
            if (
                ...
                for cont in processed_cont_great_50:
                    ...
                    size_metric = (
                        cont[8]
                        if size["name"] in ["edS", "edM", "edL", "edXL"]  # Equivalent diameter
                        else (
                            cont[9]
                            if size["name"]
                            in ["emdS", "emdM", "emdL", "emdXL"]  # Ellipse minor diameter
                            else cont[12]
                    ...
                    if (
                        shape["min"] < cont[4] <= shape["max"]
                        and size["min"] < size_metric <= size["max"]
                    ):
                        ...
                        self.processed_cont_great_50_sz[key].append(
                            [
                                ...
                                # Table II
                                (
                                    cont[7] if cont[7] <= m.atan(0.3) * 180 / m.pi else None
                                ),  # if cont[7] <= 13.5 else None,  # Irregulars theta <= 13.5°
                                (
                                    cont[7]
                                    if m.atan(0.3) * 180 / m.pi
                                    < cont[7]
                                    <= m.atan(0.5) * 180 / m.pi
                                    else None
                                ),  # if 13.5 < cont[7] <= 22.5 else None,  # Slightly irregulars
                                (
                                    cont[7]
                                    if m.atan(0.5) * 180 / m.pi
                                    < cont[7]
                                    <= m.atan(0.7) * 180 / m.pi
                                    else None
                                ),  # if 22.5 < cont[7] <= 31.5 else None,  # Slightly regulars
                                (
                                    cont[7] if m.atan(0.7) * 180 / m.pi < cont[7] else None
                                ),  # if 31.5 < cont[7] else "",  # Regulars
                                size_metric,  # Size metric (Equivalent diameter, Ellipse minor diameter, etc.)
                                (
                                    None
                                    if size["name"]
                                    in ["edS", "edM", "edL", "edXL"]  # For circular shaped pores
                                    else (
                                        cont[10]
                                        if size["name"]
                                        in [
