...
def load_image(self):
    ...
    file_paths = fd.askopenfilenames(
        title="Select Two Images", filetypes=[("Image files", "*.jpg *.jpeg *.png *.tif *.tiff")]
    )
    ...
    if len(self.images) == 2:
        ...
        combined = cv2.add(cv2.divide(self.images[0], 2), cv2.divide(self.images[1], 2))
        # Apply Gaussian blur
...
def load_mosaic(self):
    ...
    wants_roi = messagebox.askyesno(
        "Select ROI", "Do you want to select a ROI (Yes) or process the entire image (No)?"
    )
...
def save_proc_image(self, index):
    ...
    try:
        ...
        messagebox.showinfo(
            "Success", f"{self.proc_layer_names[index]} saved to '{os.path.basename(file_path)}'"
        )
    except Exception as e:
        messagebox.showerror("Error", f"Failed to save image: {str(e)}")
...
def save_mosaic_stats_data(self):
    ...
    file_path = fd.askdirectory(title="Select Directory to Save Global Pore Stats")
...
def save_original_binary(self, file_path, mosaic_name):
    ...
    try:
        ...
        messagebox.showinfo(
            "Success", f"Original binary image saved to '{os.path.basename(binary_file_path)}'"
        )
    except Exception as e:
        messagebox.showerror("Error", f"Failed to save image: {str(e)}")
...
def save_gpd_stats(self, file_path, mosaic_name):
    ...
    try:
        ...
        messagebox.showinfo(
            "Success",
            f"Global pore statistics saved to '{os.path.basename(file_path) + '/Global_Pore_Stats.xlsx'}'",
        )
    except Exception as e:
...
def save_enhanced_contours_hdf5(self, file_path, mosaic_name):
    ...
    try:
            ...
            for idx, is_edge, parent, children, area, perimeter in contour_data:
                ...
                if children:
                    ...
                    for j, child in enumerate(children):
                        children_group.create_dataset(f"{j}", data=child)
        ...
        messagebox.showinfo(
            "Success", f"Processed contours saved to '{os.path.basename(filename)}'"
        )
    except Exception as e:
    ...
    try:
        ...
        for shape in self.shapes:
            ...
            filename = os.path.join(
                file_path, mosaic_name, mosaic_name + "_" + shape["name"] + ".xlsx"
            )
            ...
            for i, size in enumerate(self.sizes):
                ...
                if (
                    ...
                    try:
                        ...
                        for row in self.processed_cont_great_50_sz.get(
                            (shape["name"], size["name"]), []
                        ):
                            ws.append(row)
                    except Exception as e:
                        messagebox.showerror(
                            "Error", f"Failed to append data to {shape['name']}: {str(e)}"
                        )
                        return
        ...
        messagebox.showerror(
            "Error", f"Failed to create xlsx file to save segmented pore info: {str(e)}"
        )
        return

    messagebox.showinfo(
        "Success",
        f"Segmented pore data saved to .xlsx files in'{os.path.basename(file_path) + '/' + mosaic_name}'",
    )
