...
def processing_tab(self):
    ...
    self.pixel_cal_input = ttk.Entry(
        self.processing_frame_controls, textvariable=self.pixel_cal, width=7, justify=tk.CENTER
    )
    self.pixel_cal_input.pack(pady=(0, 5))

    # Create a button to load the mosaic to be processed
    self.load_mosaic_button = ttk.Button(
        self.processing_frame_controls, text="Load Image", command=lambda: load_mosaic(self)
    )
    ...
    self.proc_unified_layer_frame = ttk.LabelFrame(
        self.processing_frame_controls, text="Layer Controls"
    )
    ...
    self.save_mosaic_stats_data_button = ttk.Button(
        self.proc_save_frame, text="Save Stats & Data", command=lambda: save_mosaic_stats_data(self)
    )
